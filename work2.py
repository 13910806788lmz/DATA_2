...
# ===========================================
# 读取 Parquet 文件
parquet_file_ = r'D:\360MoveData\Users\Admin\OneDrive\桌面\数据挖掘\10G_data_new\part-00000.parquet'
df = pd.read_parquet(parquet_file_).head(10000)
df = df.drop(columns=["id", "last_login", "user_name", "fullname", "email", "age", "income", "gender", "country", "address", "is_active", "phone_number"])

# 读取 mode.json 文件
with open(r'D:\360MoveData\Users\Admin\OneDrive\桌面\数据挖掘\product_catalog.json\product_catalog.json', 'r',encoding='utf-8') as f:
...
if df['categories'].apply(len).sum() > 0:
    ...
    filtered_categories = [cat for cat, count in category_counts.items() if count >= 10]
